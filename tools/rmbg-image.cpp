...
{
    if (argc < 4) {
        std::cerr << "Usage: rmbg-image MODEL.onnx INPUT.png OUTPUT.png [--device cpu|cuda|auto] [--iterations N] [--threads N]\n";
        return 2;
    }
    try {
        ...
        auto resized = image_io::resize(source, model.width(), model.height());
